...
! function( $ ) {
	...
	Codevz_Plus.grid = function( wpb ) {
		...
		$( '.cz_grid' ).not( '.slick, .slick .cz_grid' ).codevz( 'grid', function( x ) {

			var gridAnimation = function( s ) {
				...
				if ( animation ) {
					$( window ).on( 'scroll.grid_animation', function() {
						...
						items = s ? $( s ) : x.find( '.cz_grid_item:not(.' + animation + ')' );
						...
						} else if ( ! $( document.body ).hasClass( 'compose-mode' ) ) {

							$( window ).off( 'scroll.grid_animation' );
			...
			if ( x.hasClass( 'cz_justified' ) ) {
				...
				x.prev( 'ul' ).find( 'li' ).off().on( 'click', function() {
					...
					setTimeout(function() {
						...
						$( window ).trigger( 'scroll.cz_lazyload' );
					}, 1000 );
